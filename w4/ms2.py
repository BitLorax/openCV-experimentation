...
if frame is not None:
    ...
    print(img.shape)
    img = np.reshape(img, (img.shape[0] * img.shape[1], 1, 3))
    histImage = img;
    ...
    while True:
        ...
        print(track_box[1])
        # tried to split the screen in half based on the elongated ellipse
        # but couldn't find a way in time

        if showBackProj:
            vis[:] = prob[..., np.newaxis]
        try:
            cv2.ellipse(vis, track_box, (0, 0, 255), 2)
            cv2.circle(vis, center, 2, (0, 255, 0), 2)
        except:
